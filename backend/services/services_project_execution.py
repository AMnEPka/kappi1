...
class ExecutionRunContext:
    """Mutable state for a single project execution run."""
...
async def run_project_execution_events(
    ...
    The execution order and emitted payloads match the former API-local
    generator. Host tasks and scripts are still sequential.
    """
    try:
        # Get project
        ...
        yield {"type": "info", "message": f"Всего заданий: {context.total_tasks}"}

        # Process each task (each task = one host with multiple scripts)
        for task in tasks:
            async for event in execute_project_task_serial(context, task):
                yield event

        # Send completion event (don't update project status - project is reusable)
        successful_hosts = context.completed_tasks
        final_status = "completed" if context.failed_tasks == 0 else "failed"
        yield {
            "type": "complete",
            "status": final_status,
...
async def execute_project_task_serial(
    context: ExecutionRunContext,
    task: Dict[str, Any],
) -> AsyncGenerator[EventPayload, None]:
    """
    Execute one project task sequentially and yield SSE-compatible event payloads.

    This is intentionally still serial: one host task runs all preliminary checks
    and then all scripts in order. Later stages can schedule this unit in a
    bounded parallel runner without changing the event contract.
    """
