#!/usr/bin/env bash
# Исправленная версия скрипта с правильной обработкой многострочных переменных

# Используем set -e, но не -u, чтобы избежать проблем с переменными
set -eo pipefail

# CHECK_OUTPUT — уже содержит вывод команды с хоста
# ETALON_INPUT — эталонные данные

# Безопасная проверка переменных с правильной обработкой многострочных значений
if [[ -z "${CHECK_OUTPUT:-}" ]] || [[ "${CHECK_OUTPUT}" == "" ]]; then
    exit 5001 # Отсутствует переменная
fi

if [[ -z "${ETALON_INPUT:-}" ]] || [[ "${ETALON_INPUT}" == "" ]]; then
    exit 5001
fi

# Ищем строку в полученных данных (НЕ в файле!)
# ИСПРАВЛЕНО: правильное регулярное выражение [^#[:space:]] вместо [^#]
# Это означает: после пробелов должен быть символ, который НЕ является # или пробелом
