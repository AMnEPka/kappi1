...
agent_communication:
  ...
  - agent: "main"
    message: "🔧 SSE ROUTING FIX: Fixed 404 error on project execution. Root cause: 1) api.js was using hardcoded port 8001 instead of REACT_APP_BACKEND_URL from .env 2) EventSource was using relative URL without proper backend URL 3) EventSource doesn't support Authorization headers. Solution: 1) Updated api.js to use REACT_APP_BACKEND_URL 2) Modified SSE endpoint to accept token via query parameter 3) Added get_current_user_from_token() auth function for SSE 4) Frontend now passes token in query string. Both regular API calls and SSE connections now properly route to backend."